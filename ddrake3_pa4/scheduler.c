 ...
 * implement the scheduling for the round robin scheduler
 *
 ***************************************************************************************/
...
#define SIG SIGRTMIN // real-time signal
...
// structure to change the action taken by a process on receipt of a specific signal
...
static void init_sched_queue(int queue_size)
{
	...
	timer_create(CLOCK, &sevent, &timer);
}
